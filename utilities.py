...
def plot_(array, theta, bin_width  = 0.01, r_range = np.array([0.2, 0.5]), name_1 = "None", name_2 = "None"):
        fig, ax = plt.subplots()
        ...
        '''
        V = (4/3)* (np.pi) * (np.power(edges[1:], 3) - np.power(edges[:-1], 3))
        norm = 3 *V # Use real normalization factor in comments
        try:
...
def hist(array, bin_width  = 0.01, r_range = np.array([0.2, 0.5])):
    n_bins = int((r_range[1] - r_range[0]) / bin_width)
    g_rst, edges = np.histogram(array, range=r_range, bins=n_bins)
    r = 0.5 * (edges[1:] + edges[:-1]) 
    '''
