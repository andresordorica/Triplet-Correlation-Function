...
def fast_tcf_self(u, atom_name_1,stride = 1000, theta = 60, plot=False, demo=True, chunk = 100):
    '''
    ...
    if demo ==False:
        ...
        positions1 = positions1.astype(np.float64)
        ...
        data_type = positions1.dtype

    valid_distances, valid_angles = _tcf_self(positions1, tolerance = 0.010, angle = theta)
        
    if plot==True:
        if demo == True:
            plot_(valid_distances, theta, bin_width  = 0.01, r_range = np.array([0.2, 0.5]), name_1 = atom_name_1.split()[1], name_2 = atom_name_1.split()[1], ucv = None)
        elif demo == False:
            unit_cell = (u.trajectory.ts.triclinic_dimensions)/10
            unit_cell_volumes_ = (np.abs(np.linalg.det(unit_cell)))
            unit_cell_volumes = np.repeat(unit_cell_volumes_, int(u.trajectory.n_frames))
            plot_(valid_distances, theta, bin_width  = 0.01, r_range = np.array([0.2, 0.5]), name_1 = atom_name_1.split()[1], name_2 = atom_name_1.split()[1], ucv = unit_cell_volumes)

    
    return valid_distances, valid_angles


def fast_tcf_(u, atom_name_1,atom_name_2,stride = 1000, theta = 60, plot=False, demo=True, chunk = 100, chunk_solvent = 100):
    if demo == False:
        '''
        ...
        for i, ts in enumerate(u.trajectory):
            if i % stride ==0:
                ...
                n_rows1 = coordinates_1.shape[0]
                t_1 = int(n_rows1*(chunk/100))
                if n_rows1 > t_1:
                    coordinates_1 = coordinates_1[:t_1]
                if i ==0:
                ...
                t_ = int(n_rows2*(chunk_solvent/100))
                # Select the first 2000 rows if there are more than 2000 rows
                if n_rows2 > t_:
                    coordinates_2 = coordinates_2[:t_]
                ###########################################################
    ...
    if plot==True:
        if demo == True:
            plot_(valid_distances, theta, bin_width  = 0.01, r_range = np.array([0.2, 0.5]), name_1 = atom_name_1.split()[1], name_2 = atom_name_2.split()[1], ucv = None)
        elif demo == False:
            unit_cell = (u.trajectory.ts.triclinic_dimensions)/10
            unit_cell_volumes_ = (np.abs(np.linalg.det(unit_cell)))
            unit_cell_volumes = np.repeat(unit_cell_volumes_, int(u.trajectory.n_frames))
            plot_(valid_distances, theta, bin_width  = 0.01, r_range = np.array([0.2, 0.5]), name_1 = atom_name_1.split()[1], name_2 = atom_name_2.split()[1], ucv =unit_cell_volumes )
